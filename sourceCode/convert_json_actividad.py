...
def get_fields_json(name_file):
	...
	#Creo el archivo CSV
	csv_file = open('../data/TripAdvisor/pdata_final_actividades_co.csv', 'w')
	#Creo la cabecera del CSV
	cabecera_json = 'NAME,USER,LONGITUDE,LATITUDE,RATING,NoREVIEWS,HELPFUL,NoOpinionsHotels,CITY,COUNTRY,TAGS\n'
						...
						if user:
							...
							if location_user:
								...
								if helpful:
									...
									line = str(name)+','+str(user)+','+str(lng)+','+str(lat)+','+str(rating)+','+str(NoReviews)+','+str(helpful)+','+str(opinions)+','+str(city)+','+str(country)+','+line_tags+'\n'
									#Guardo la linea en el archivo CSV
									csv_file.write(line)

if __name__ == '__main__':
	get_fields_json('scrapy_hotel/scrapy_hotel/spiders/actividades_data_co.json')
