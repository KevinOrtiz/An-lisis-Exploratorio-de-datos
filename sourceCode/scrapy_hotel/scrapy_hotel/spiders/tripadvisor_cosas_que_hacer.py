...
class tripAdvisorScrapper(CrawlSpider):
	...
	start_urls = ['https://www.tripadvisor.co/Attractions-g294308-Activities-Quito_Pichincha_Province.html']

	rules = (
		Rule(LinkExtractor(restrict_xpaths=".//div[contains(@class, 'pagination')]//a"), follow = True),
		Rule(LinkExtractor(allow=['/Attraction_Review-g.*Quito_Pichincha_Province.*'], unique=True), callback='parse_items', follow = True),
	) 
	...
	def parse_items(self, response):
		...
		if expanded_review_url:
			...
			yield Request(url=url_review, meta={'cosas_que_hacer_item': l, 'counter_page_review' : 0}, callback=self.parse_fetch_review)

	def parse_fetch_review(self, response):
		
		cosas_que_hacer_item = response.meta['cosas_que_hacer_item']
		sel = scrapy.selector.Selector(response)
		...
		if counter_page_review < MAX_REVIEWS_PAGES:
			...
			if len(snode_reviews) > 0:
				...
				cosas_que_hacer_item.get_collected_values('review').append(tripadvisor_review_item)
			...
			if next_page_url and len(next_page_url) > 0:
				yield Request(url=self.base_uri + next_page_url, meta={'cosas_que_hacer_item': cosas_que_hacer_item, 'counter_page_review' : counter_page_review}, callback=self.parse_fetch_review)
			else:
				yield cosas_que_hacer_item.load_item()
