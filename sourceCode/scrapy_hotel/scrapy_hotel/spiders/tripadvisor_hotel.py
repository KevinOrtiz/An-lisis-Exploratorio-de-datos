...
class tripAdvisorScrapper(BaseSpider):
	...
	def parse(self, response):
		...
		if counter_page_hotels < MAX_REVIEWS_PAGES:
			...
			if next_page_hotels and len(next_page_hotels) > 0:
				...
				yield Request(url=url_hotels, cookies={'store_language':'en'}, meta={'counter_page_hotels' : counter_page_hotels}, callback=self.parse)
	...
	def parse_search_page(self, response):
		...
		tripadvisor_item['rating'] = clean_parsed_string(get_parsed_rating(sel, '//*[@id="HEADING_GROUP"]//div[contains(@class, "rs rating")]/span[contains(@class,"rate sprite-rating_rr")]/img/@alt'))
	...
	def parse_fetch_review(self, response):
		...
		if counter_page_review < MAX_REVIEWS_PAGES:
			...
			for snode_review in snode_reviews:
				...
				tripadvisor_review_item['date_review'] = clean_parsed_string(get_parsed_date(snode_review, './/div[@class="rating"]/span[@class="ratingDate"]/text()'))
